...
###################################################
  ...
  global dpt2m_1a,dpt2m_1b,dpt2m_2,dpt2m_3,dpt2m_4a,dpt2m_4b

# 2-m Dewpoint Temperature
  if not plot_nodata_text[0]:
      dpt2m_1a = data1a.select(shortName='DPT',level='2 m above ground')[0].data
      dpt2m_1a = (dpt2m_1a - 273.15)*1.8 + 32.0
  if not plot_nodata_text[1]:
      dpt2m_1b = data1b.select(shortName='DPT',level='2 m above ground')[0].data
      dpt2m_1b = (dpt2m_1b - 273.15)*1.8 + 32.0
  if not plot_nodata_text[2]:
      dpt2m_2 = data2.select(shortName='DPT',level='2 m above ground')[0].data
      dpt2m_2 = (dpt2m_2 - 273.15)*1.8 + 32.0
  if not plot_nodata_text[3]:
      dpt2m_3 = data3.select(shortName='DPT',level='2 m above ground')[0].data
      dpt2m_3 = (dpt2m_3 - 273.15)*1.8 + 32.0
  if not plot_nodata_text[4]:
    # URMA
      dpt2m_4a = data4a.select(shortName='DPT',level='2 m above ground')[0].data
      dpt2m_4a = (dpt2m_4a - 273.15)*1.8 + 32.0
  if not plot_nodata_text[5]:
    # RTMA
      dpt2m_4b = data4b.select(shortName='DPT',level='2 m above ground')[0].data
      dpt2m_4b = (dpt2m_4b - 273.15)*1.8 + 32.0
      ...
      for anl in datasets:
        ...
        if anl == 'URMA':
            ...
            if use_mod1 == 'a' and plot_nodata_text[0]:
              ...
              cs_1 = ax1.pcolormesh(use_lon1_shift,use_lat1_shift,use_dpt2m_1,transform=transform,cmap=cm,norm=norm)
            ...
            if plot_nodata_text[2]:
              ...
              cs_2 = ax2.pcolormesh(lon2_shift,lat2_shift,dpt2m_2,transform=transform,cmap=cm,norm=norm)
            ...
            if plot_nodata_text[3]:
              ...
              cs_3 = ax3.pcolormesh(lon3_shift,lat3_shift,dpt2m_3,transform=transform,cmap=cm,norm=norm)
            ...
            if plot_nodata_text[4]:
              ...
              cs_4 = ax4.pcolormesh(lon4a_shift,lat4a_shift,dpt2m_4a,transform=transform,cmap=cm,norm=norm)
            ...
            if plot_nodata_text[5]:
              ...
              cs_4 = ax4.pcolormesh(lon4b_shift,lat4b_shift,dpt2m_4b,transform=transform,cmap=cm,norm=norm)
            cbar4 = fig.colorbar(cs_4,ax=ax4,orientation='horizontal',pad=0.01,shrink=1.0,extend='both')
            cbar4.set_label(units,fontsize=6,labelpad=0)
            cbar4.ax.xaxis.set_tick_params(pad=0)
            cbar4.ax.tick_params(labelsize=6)
        ...
        rrfs_plot_utils.convert_and_save(f'comparedpt2m{use_mod1}_'+dom+'_f'+fhour+'_'+anl)
        t2 = time.perf_counter()
        t3 = round(t2-t1, 3)
        print(('%.3f seconds to plot 2-m Dewpoint Temperature with '+anl+' for: '+dom+f'model{use_mod1}') % t3)
